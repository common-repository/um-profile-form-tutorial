...
(function(context, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    var namespace = 'borohop';

    // Browser globals

    if (context[namespace]) {

      // borohop already exists.
  ...
  try {
    ...
    if(typeof window.sessionStorage !== undefinedStr){
      ...
      sessionStorage.setItem('borohop.test.storage', 'ok');

      sessionStorage.removeItem('borohop.test.storage');
  ...
  utils = {
    ...
    getI18NString: function(key) {
      ...
      return customI18N[key] || borohopI18N[key];
   ...
   * borohop.registerHelper() and borohop.unregisterHelper().
   ...
   * @class The borohopBubble class represents the view of a bubble. This class is also used for borohop callouts.

   */

  borohopBubble = function(opt) {
  ...
  borohopBubble.prototype = {
    ...
    render: function(step, idx, callback) {
      ...
      if(this.opt.isTourBubble){
        ...
        currTour = winborohop.getCurrTour();
      ...
      if(typeof tourSpecificRenderer === 'function'){
        ...
        if(!winborohop.templates || (typeof winborohop.templates[tourSpecificRenderer] !== 'function')){

          throw new Error('Bubble rendering failed - template "' + tourSpecificRenderer + '" is not a function.');

        }

        el.innerHTML = winborohop.templates[tourSpecificRenderer](opts);
        ...
        if(!winborohop.templates || (typeof winborohop.templates[templateToUse] !== 'function')){

          throw new Error('Bubble rendering failed - template "' + templateToUse + '" is not a function.');

        }

        el.innerHTML = winborohop.templates[templateToUse](opts);
      ...
      for (i = 0; i < numChildren; i++){
        ...
        if(utils.hasClass(node, 'borohop-arrow')){
    ...
    _getStepNum: function(idx) {
      ...
      var skippedStepsCount = 0,
          ...
          skippedSteps = winborohop.getSkippedStepsIndexes(),
    ...
    _handleBubbleClick: function(evt){
      ...
      function findMatchRecur(el){
         ...
         * borohop-cta, borohop-next, borohop-prev, borohop-close

         */

         if(el === evt.currentTarget){ return null; }

         if(utils.hasClass(el, 'borohop-cta')){ return 'cta'; }

         if(utils.hasClass(el, 'borohop-next')){ return 'next'; }

         if(utils.hasClass(el, 'borohop-prev')){ return 'prev'; }

         if(utils.hasClass(el, 'borohop-close')){ return 'close'; }
      ...
      if (action === 'cta'){
        ...
        if (!this.opt.isTourBubble) {
          ...
          winborohop.getCalloutManager().removeCallout(this.currStep.id);
        ...
        winborohop.nextStep(true);

      }

      else if (action === 'prev'){

        winborohop.prevStep(true);
        ...
        if (this.opt.isTourBubble){
          ...
          var currStepNum   = winborohop.getCurrStepNum(),
          ...
          winborohop.endTour(true, doEndCallback);
          ...
          if (this.opt.id && !this.opt.isTourBubble) {
            ...
            // Remove via the borohopCalloutManager.

            // removeCallout() calls borohopBubble.destroy internally.

            winborohop.getCalloutManager().removeCallout(this.opt.id);
    ...
    init: function(initOpt) {
      ...
      el.className = 'borohop-bubble animated';

      if (!opt.isTourBubble) {

        utils.addClass(el, 'borohop-callout no-number');
  ...
  borohopCalloutManager = function() {
     ...
     * borohop.refreshBubblePosition() to refresh ALL
   ...
   * @class Creates the borohop object. Used to manage tour progress and configurations.
  ...
  borohop = function(initOptions) {
     ...
     * Destroy the bubble currently associated with borohop.
    ...
    setStateHelper = function() {
      ...
      var cookieVal = currTour.id + ':' + currStepNum,
        ...
        skipedStepIndexes = winborohop.getSkippedStepsIndexes();
     ...
     * Initializes the borohop object.
    ...
    init = function(initOptions) {
      ...
      if (initOptions) {
        ...
        //initOptions.cookieName = initOptions.cookieName || 'borohop.tour.state';
     ...
     * @returns {Object} borohopCalloutManager
    ...
    this.getCalloutManager = function() {
      ...
      if (typeof calloutMgr === undefinedStr) {
        ...
        calloutMgr = new borohopCalloutManager();
     ...
     * Tell borohop that the position of the current tour element changed

     * and the bubble therefore needs to be redrawn. Also refreshes position

     * of all borohop Callouts on the page.
     ...
     * Remove callbacks for borohop events. If tourOnly is set to true, only

     * removes callbacks specified by a tour (callbacks set by external calls

     * to borohop.configure or borohop.listen will not be removed). If
     ...
     * @example borohop.configure({ scrollDuration: 1000, scrollTopMargin: 150 });
     ...
     * Set the template that should be used for rendering borohop bubbles.

     * If a string, it's assumed your template is available in the

     * borohop.templates namespace.

     *

     * @param {String|Function(obj)} The template to use for rendering.

     * @returns {Object} The borohop object (for chaining).
...
__p += '\n<div class="borohop-bubble-container" style="width: ' +
...
__p += '<span class="borohop-bubble-number">' +
...
__p += '\n  <div class="borohop-bubble-content">\n    ';

 if(step.title !== ''){ ;

__p += '<h3 class="borohop-title">' +
...
__p += '\n  </div>\n  <div class="borohop-actions">\n    ';

 if(buttons.showPrev){ ;

__p += '<button class="borohop-nav-button prev borohop-prev">' +
...
__p += '<button class="borohop-nav-button next borohop-cta">' +
...
__p += '<button class="borohop-nav-button next borohop-next">' +
...
__p += '<button class="borohop-bubble-close borohop-close">' +
...
__p += '\n</div>\n<div class="borohop-bubble-arrow-container borohop-arrow">\n  <div class="borohop-bubble-arrow-border"></div>\n  <div class="borohop-bubble-arrow"></div>\n</div>';
